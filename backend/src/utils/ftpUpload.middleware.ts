...
import { uploadFileToFTP } from './ftpUpload.util';
import os from 'os';
...
/**
 * Middleware to handle file uploads via FTP
 * 1. First saves files to temporary storage using multer
 * 2. Then uploads files to FTP server
 * 3. Adds file information to request object
 * 4. Cleans up temporary files
 */
export const ftpUploadMiddleware = (fieldName: string = 'images', maxCount: number = 10) => {
    ...
    multerMiddleware(req, res, async (err) => {
      ...
      try {
        ...
        const uploadPromises = req.files.map(async (file: Express.Multer.File, index: number) => {
          // Upload to FTP
          const ftpResult = await uploadFileToFTP(file.path, 'vehicles');
          
          // Add additional information to the file object
          const enhancedFile = {
            ...file,
            path: ftpResult.url, // Replace local path with FTP URL
            location: ftpResult.url, // For compatibility with S3-style code
            key: ftpResult.key,
            publicId: ftpResult.publicId,
        ...
        console.error('FTP upload middleware error:', error);
...
export default ftpUploadMiddleware; 